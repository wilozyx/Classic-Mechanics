...
execute if score CM.global CM_SearchIndex >= CM.global CM_SearchTotalEntries run \
    return run \
        tellraw @a[tag=CM.PlayerSearchedModules] {"click_event":{"action":"run_command","command":"/function classic_mechanics:main/search/option_page/page_open"},"color":"#E75C5E","hover_event":{"action":"show_text","value":[{"text":">Click to return to search menu","color":"#EB5050"}]},"text":"[← Return to the search menu]"}
