...
execute if block ~ ~ ~ wet_sponge run \
    setblock ~ ~ ~ sponge strict

# -> Replace nearby water with light blocks
execute store success score CM.global CM_MathDynamicValue run \
    fill ~-2 ~-2 ~-2 ~2 ~2 ~2 air replace #classic_mechanics:old_sponges/sponge_drainable strict
...
execute if score CM.global CM_MathDynamicValue matches 0 unless block ~ ~1 ~ air run \
