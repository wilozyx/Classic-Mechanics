...
# -> If ridden boat is above water then return and display adequate water particles
execute on vehicle at @s if block ~ ~-0.2 ~ minecraft:water run \
    return run \
        particle minecraft:splash ^ ^0.5 ^-0.8 0.3 0 0.3 0.1 10 normal

# -> Else, display other set of the same particles
execute on vehicle at @s run \
    particle minecraft:splash ^ ^ ^-0.5 0.3 0 0.3 0.1 5 normal