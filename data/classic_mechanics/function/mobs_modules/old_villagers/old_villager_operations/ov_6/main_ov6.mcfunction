...
# -> If villager does not have its inventory disabled then disable it
execute if entity @s[tag=!CM.OV.DisabledInventory] run \
    function classic_mechanics:mobs_modules/old_villagers/disable_inventory

# -> If villager already has the DontCalc tag then return
