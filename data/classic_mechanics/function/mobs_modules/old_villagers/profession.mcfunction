...
$data merge entity @s {VillagerData:{level:99,profession:"minecraft:$(runprof)"},Tags:["CM.OV.DontCalc"]}
execute if score CM.global CM_OldVillagers matches 6..7 if data entity @s {VillagerData:{profession:"minecraft:cartographer"}} run data modify entity @s VillagerData.profession set value "minecraft:librarian"
$execute if score CM.global CM_OldVillagers matches 3.. run item replace entity @s weapon.offhand with minecraft:stick[minecraft:enchantments={vanishing_curse:1},minecraft:custom_data={profid:$(runid)}]
execute if score CM.global CM_OldVillagers matches 6.. run scoreboard players add @s CM_OV_Level 1
